...
export default {
  ...
  methods: {
    initScene() {
      ...
      // Arcos + marcadores de origem + pulso viajante
      ...
      CITIES.forEach((city, i) => {
        ...
        pulse.scale.set(0.09, 0.09, 0.09)
        globeGroup.add(pulse)

        travelers.push({ curve, sprite: pulse, offset: i * 0.35, speed: 0.25 })
      })

      this.three = { scene, camera, renderer, controls, globeGroup, travelers }
      ...
      const animate = () => {
        ...
        travelers.forEach((tr) => {
          const progress = ((t * tr.speed + tr.offset) % 1)
          const p = tr.curve.getPoint(progress)
          tr.sprite.position.copy(p)
          const fade = Math.sin(progress * Math.PI)
          tr.sprite.material.opacity = 0.15 + fade * 0.85
        })
        controls.update()
