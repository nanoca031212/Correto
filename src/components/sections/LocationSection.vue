  ...
  <section id="localizacao" class="location section">
    <div class="container">
      ...
      <div class="section-header">
        ...
        <SplitText tag="p" class="section-subtitle" :delay="8">
      ...
      <div class="location-content">
        ...
        <div class="location-info">
          ...
          <div class="points-of-interest">
            ...
            <div class="poi-grid" ref="poiGridEl">
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      if (poiGridEl.value) {
        const items = Array.from(poiGridEl.value.querySelectorAll('.poi-item'))

        if (reduceMotion) {
          items.forEach((item) => item.classList.add('is-visible'))
        } else {
          // Cada item é observado individualmente (não o grid inteiro), pra
          // animar um por vez conforme entra na tela. Sem "once": ao sair de
          // tela (ex.: scroll para cima) volta ao estado inicial e reanima.
          poiObserver = new IntersectionObserver((entries) => {
            entries.forEach((entry) => {
              entry.target.classList.toggle('is-visible', entry.isIntersecting)
            })
          }, { threshold: 0.2 })
          items.forEach((item) => poiObserver.observe(item))
        }
      }
    })
